...
---
# My Goals for Today
* Highlight some of the **data problems** we face as scientists.
* **Illustrate** one way to tackle the issues.
* Hopefully, get your **creative juices flowing**.
...
.pull-right[
    <img src = 'figures/stopconfusingmewithfacts.jpg' width = 100%/>
]
...
pHOS <- cleaned_car %>%
  filter(CarcassSpecies == 'S_CHN',
         ...
         AboveWeir == 'Yes') %>%
  est_group_p(.summary_var = Origin,
              alpha = 0.05,
              SurveyYear,
              StreamName) %>%
...
pPSM <- cleaned_car %>%
  filter(CarcassSpecies == 'S_CHN',
         ...
         AboveWeir == 'Yes') %>%
  est_group_p(.summary_var = SpawnedOut,
              alpha = 0.05,
              SurveyYear,
              StreamName) %>%
...
NOSA <- left_join(
  ...
  left_join(
    pPSM %>%
      filter(SpawnedOut == 'No'),
    by = c("SurveyYear", "StreamName")) %>%
  mutate(NOSA = N * (1-pHOS_p) * (1-pPSM_p))
```
...
# Natural-origin Spawner Abundance - Take 2
...
NOSA_ests <- get_NOSAests(cleaned_redd, cleaned_car, MR_ests)
```

```{r NOSA_fig_2, fig.show = 'hide'}
NOSA_ests %>%
